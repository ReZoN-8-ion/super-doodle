...
describe('ggml Hybrid Persona Agent', () => {
  ...
  describe('Repository Spawning', () => {
    ...
    it('should respect recursion depth limit', async () => {
      ...
      await expect(personaAgent.spawnRepository(spawnRequest)).rejects.toThrow('Maximum recursion depth 3 exceeded');
    });
    ...
    it('should generate correct dependencies for technologies', async () => {
      ...
      const result = await personaAgent.spawnRepository(spawnRequest);
      const packageJsonAction = result.boltArtifact.actions.find((action: any) => action.filePath === 'package.json');
  ...
  describe('Error Handling and Resilience', () => {
    it('should handle invalid recursion gracefully', async () => {
      ...
      await expect(personaAgent.spawnRepository(invalidRequest)).rejects.toThrow('Maximum recursion depth');
    });
  ...
  describe('Performance and Optimization', () => {
    it('should process multiple intents efficiently', async () => {
      ...
      const results = await Promise.all(intents.map((intent) => personaAgent.processIntent(intent)));
